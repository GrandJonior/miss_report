<?php 
	...
	$list =2;
 	...
 	foreach ($results as $result) {
 		...
 		foreach ($final_orders as $final_order ) {
				...
				$insert_order['original_order_sn'] = $original_order['order_sn'];
				$insert_order['original_deal_code'] = $original_order['deal_code'];
				$insert_order['pay_time'] = $pay_time;
				$insert_order['channel'] = $tel;
				$insert_order['original_sku_count'] = $original_order['sku_count'];
				$insert_order['original_shop_goods_amount'] = $original_order['shop_goods_amount'];
				$insert_order['final_order_sn'] = $final_order['order_sn'];
				$insert_order['final_deal_code'] = $final_order['deal_code'];
				$insert_order['final_order_stauts'] = $final_order['order_status'];
				$insert_order['final_sku_count'] = $final_order['sku_count'];
				$insert_order['final_shop_goods_amount'] = $final_order['shop_goods_amount'];
				$insert_order['rate'] = $rate;
				// echo '<pre>';
				// print_r($insert_order);
				$res = $db->insertInto($insert_order,$dbname);
				// print_r($res) ; die;  	
				$list++;
	...
	browser_export('Excel5','Miss报表'.time().'.xls');//输出到浏览器
	...
	function browser_export($type,$filename){
 	...
 	function sort_Realloc(&$realloc_orders = array()){
		...
		foreach($realloc_orders AS $realloc_order){  
		       ...
		       $order_sns[] = $realloc_order['order_sn'];
		    
		}  
	
		array_multisort($order_sns, SORT_ASC, $realloc_orders);  
